...
async function sendEmail(to: string, name: string, resetUrl: string) {
  ...
  try {
    const res = await fetch('https://api.brevo.com/v3/smtp/email', {
      ...
      body: JSON.stringify({ sender:{ name:'DVL Dashboard — IFHE', email:'noreply@sanjayfuloria.tech' }, to:[{ email:to, name }], subject:'DVL Dashboard — Password Reset', htmlContent:html }),
    })
    const data = await res.json()
    if (data.messageId) return { ok: true, provider: 'brevo' }
  } catch(e) {}
